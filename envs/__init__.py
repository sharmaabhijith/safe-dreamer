...
def make_env(config, id):
    ...
    if suite == "dmc":
        ...
        env = DistractingControl(
            ...
            floor_video=getattr(config, "floor_video", False),
            floor_video_alpha=getattr(config, "floor_video_alpha", 1.0),
            ground_plane_alpha=getattr(config, "ground_plane_alpha", None),
